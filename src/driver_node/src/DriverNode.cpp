...
DriverNode::DriverNode() : Node("driver_node"), x_(0.0), y_(0.0), th_(0.0), vx_(0.0), vy_(0.0), wz_(0.0)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Motor params: K=%.3f, speed_scale=%.1f, min_pwm=%d, deadzone=%d, max_pwm=%d, cmd_timeout=%.2fs", robot_->K_, robot_->speed_scale_, robot_->min_pwm_, robot_->pwm_deadzone_, robot_->max_pwm_, cmd_timeout_);
    ...
    param_cb_handle_ = this->add_on_set_parameters_callback(
        [this](const std::vector<rclcpp::Parameter> &params) {
            ...
            RCLCPP_INFO(this->get_logger(), "Motor params updated: K=%.3f, speed_scale=%.1f, min_pwm=%d, deadzone=%d, max_pwm=%d, cmd_timeout=%.2fs", robot_->K_, robot_->speed_scale_, robot_->min_pwm_, robot_->pwm_deadzone_, robot_->max_pwm_, cmd_timeout_);
            return res;
        }
    );
...
void DriverNode::check_timeout()
{
    ...
    if ((this->get_clock()->now() - last_command_time_).seconds() > cmd_timeout_) {
        vx_ = 0.0;
        vy_ = 0.0;
        wz_ = 0.0;
        robot_->stop();

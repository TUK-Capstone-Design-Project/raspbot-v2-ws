...
KinematicsInterface::KinematicsInterface(const std::string &device, uint8_t addr) : fd_(-1), i2c_device_(device), i2c_addr_(addr)
{
...
KinematicsInterface::KinematicsInterface(rclcpp::Node::SharedPtr node, const std::string &device, uint8_t addr) : fd_(-1), i2c_device_(device), i2c_addr_(addr)
{
...
void KinematicsInterface::tick()
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        double weighted = intended_linear_pwm_[i] * linear_gain_ + intended_angular_pwm_[i] * angular_gain_;

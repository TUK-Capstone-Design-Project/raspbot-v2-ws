...
public:
     ...
     */
    KinematicsInterface(const std::string &device = "/dev/i2c-1", uint8_t addr = 0x2B);

    KinematicsInterface(rclcpp::Node::SharedPtr node, const std::string &device = "/dev/i2c-1", uint8_t addr = 0x2B);
